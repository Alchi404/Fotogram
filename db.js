let myImgs = [
"./assets/img/01_erstes_Compsit.webp",
"./assets/img/02_Duplication.webp",
"./assets/img/03_Eulenkatzeeee.webp",
"./assets/img/04_Gondor_Polaris2.webp",
"./assets/img/05_Guts_polaris.webp",
"./assets/img/06_hasi.webp",
"./assets/img/07_Hornet_polaris2.webp",
"./assets/img/08_Link_comkposite.webp",
"./assets/img/09_Majoras_Maske_polaris2.webp",
"./assets/img/10_legger.webp",
"./assets/img/11_Pyramid_head_Polaris2.webp",
"./assets/img/12_Rostock_Natur.webp",
]

let myImgsAlt = [
    "Image of erstes_Compsit",
    "Image of Duplication",
    "Image of Eulenkatzeeee",
    "Image of Gondor_Polaris",
    "Image of Guts_polaris",
    "Image of hasi",
    "Image of Hornet_polaris",
    "Image of Link_comkposite",
    "Image of Majoras_Maske_polaris",
    "Image of an expensive Sandwich",
    "Image of Pyramid_head_Polaris",
    "Image of Rostock_Natur",
]

let myImgsTitle = [
    "My first compsit, yaaay",
    "Me and Me and me an...",
    "Whooo ?",
    "For Gondor!",
    "Guts of Berserk",
    "Yawning Bunny",
    "what a nice Bug",
    "Hey Listen!",
    "is this Zelda ?",
    "most expensive Sandwich 2026",
    "one word... Neckpain",
    "Beautiful City",
]